...
function queryDeclare(){
	...
	var pageFormObj = document.getElementById("myPageForm");
	pageFormObj.action="<%=path%>/dealer/filingProposalManager/filingProposalRepairList.vti?startIndex=${transMsg.startIndex}";
    pageFormObj.submit();	
	//$("#myPageForm").submit();
}
...
function cancelFilingProposal(filingProposalId){
	var pageFormObj = document.getElementById("myPageForm");
	var actionUrl="<%=path%>/dealer/filingProposalManager/filingProposalRepairList.vti?startIndex=${transMsg.startIndex}";
	$.ajax({
		url:"<%=path%>/dealer/filingProposalManager/cancelFilingProposal.vti",
		type:"POST",
		dataType:"json",
		data:{filingProposalId:filingProposalId,filingProposalType:2},
...
<div class="bd_rt" id="audit">
	<div class="bd_rt_nav clearfix">
		...
		<c:if test="${menuType == 0}" >
			<div ><span>当前位置 ： 报备管理  > 事故维修</span></div>
		</c:if>
	</div>
	<div class="audit_nav_c">
	    <strong><span><a href="#" onClick="javascript:gotoLoadExcel();">表格导出</a>
	    <a href="javascript:openWindow('<%=path%>/dealer/filingProposalManager/addFilingProposalView.vti?filingProposalType=2', 780, 450, '添加事故维修')">添加事故维修</a>
	    </span></strong>
	        网络代码
		...
		<div class="table_list table_all" >
			...
			<ul class='table_bd clearfix ${statusObj.index%2==0?"even":"odd"}' style="min-width:1600px">
            	...
            	<li style="width:320px">
		 			...
		 			<a  onClick="cancelFilingProposal('<c:out value="${filingProposal.filingProposalId}" />');">取消事故维修</a>
					<a href="javascript:openWindow('<%=path%>/dealer/filingProposalManager/editFilingProposalView.vti?filingProposalId=<c:out value="${filingProposal.filingProposalId}" />&filingProposalType=2', 780, 400, '编辑事故维修')">编辑事故维修</a>
					...
					<a href="javascript:openWindow('<%=path%>/dealer/filingProposalManager/filingProposalDetailView.vti?filingProposalId=<c:out value="${filingProposal.filingProposalId}" />&filingProposalType=2', 780, 400, '查看详情')">查看详情</a>
            	</li>
			</ul>
		</c:forEach>
		</c:if>
