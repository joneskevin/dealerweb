...
<script>
	...
	function query(){
		...
		if (startTime == "" || endTime == "") {
			alert("请输入试驾的开始时间和结束时间");
			return;
		}
		pageFormObj.submit();
	}
	function gotoLoadExcel(){
		pageFormObj.action="<%=path%>/dealer/testDrive/outputIntevel.vti?startIndex=${transMsg.startIndex}";
	    pageFormObj.submit();
	    pageFormObj.action="<%=path%>/dealer/testDrive/listInterval.vti";
	}    
</script>

<body>
<form id="myPageForm" action="<%=path%>/dealer/testDrive/listInterval.vti" method="get" >
<div class="bd_rt" id="audit">
	...
	<div class="audit_nav_c">
		...
		试驾时间
		<input type="text" id="startTime" name="startTime"  onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',autoPickDate:true})" value="<fmt:formatDate value="${testDrive.startTime}" type="both" pattern="yyyy-MM-dd HH:mm:ss" />" class="Wdate right" readonly="true"/>
		-
		<input type="text" id="endTime" name="endTime"  onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',autoPickDate:true})" 	value="<fmt:formatDate value="${testDrive.endTime}" type="both" pattern="yyyy-MM-dd HH:mm:ss" />" class="Wdate right" readonly="true"/>
					
		<input type="button"  class="btn_submit" onclick="query()" value="查询" />
	</div>
				...
				<c:forEach var="vo" items="${testDriveVOList}" varStatus="statusObj">
					<ul class='table_bd clearfix ${statusObj.index%2==0?"even":"odd"}'style="width:1360px;">
	            		...
	            		<li class="plateNumber_75"><span title="${vo.plateNumber}">${vo.plateNumber}</span></li>
		           		...
		           		<li class="mileageTotal_80"><span title="<fmt:formatNumber type="number" maxFractionDigits="1" value="${vo.mileageTotal/1000}" />">
			           		<fmt:formatNumber type="number" maxFractionDigits="1" value="${vo.mileageTotal/1000}" /></span>
		           		</li>
		           		<li class="short_operator_150">
							...
							&vehicleId=${vo.vehicleId}&lineId=${vo.lineId}&startTime=${startTime}&endTime=${endTime}&higherPage=${transMsg.startIndex}">查看明细</a>
		           		</li>
					</ul>
				</c:forEach>
			</c:if>
