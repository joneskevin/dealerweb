...
function gotoLoadExcel(){
	pageFormObj.action="<%=path%>/dealer/violation/outputWeek.vti?startIndex=${transMsg.startIndex}";
    pageFormObj.submit();
    pageFormObj.action="<%=path%>/dealer/violation/listWeek.vti?startIndex=${transMsg.startIndex}";
}
</script>

<body>
<form id="myPageForm" action="<%=path%>/dealer/violation/listWeek.vti" method="get" >
<div class="bd_rt" id="audit">
	...
	<div class="audit_nav_c">
		...
		违规周期(本年)：
		 <form:input path="violation.week" cssClass="ipt_txt"/>周
		...
		<div class="table_list table_all" style="min-width:1200px;overflow-y:hidden;overflow:auto">
				...
				<ul class='table_bd clearfix ${statusObj.index%2==0?"even":"odd"}'style="width:1330px;">
            		...
            		<li class="tiny_operator_100">
				         <a href="<%=path%>/dealer/violation/detail.vti?vehicleId=<c:out value="${violation1.vehicleId}" />&driveLineId=
						<c:out value="${violation1.driveLineId}"/>&week=<c:out value="${violation.week}" /> 
						&typeId_nick=<c:out value="${violation1.typeId_nick}"/>&vin=<c:out value="${violation1.vin}" />
						&plateNumber=<c:out value="${violation1.plateNumber}" />&carStyleId_Nick=<c:out value="${violation1.carStyleId_Nick}" />
						...
						&companyName=<c:out value="${violation1.companyName}" /> ">查看明细</a>
					</li>
			</ul>
		</c:forEach>
		</c:if>
