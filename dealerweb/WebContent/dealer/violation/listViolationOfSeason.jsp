...
function gotoLoadExcel(){
	pageFormObj.action="<%=path%>/dealer/violation/outputSeason.vti?startIndex=${transMsg.startIndex}";
    pageFormObj.submit();
    pageFormObj.action="<%=path%>/dealer/violation/listSeason.vti?startIndex=${transMsg.startIndex}";
}
</script>

<body>
<form id="myPageForm" action="<%=path%>/dealer/violation/listSeason.vti" method="get" >
<div class="bd_rt" id="audit">
	...
	<div class="audit_nav_c">
			...
			<form:select path='violation.season'  cssClass="select_condition">
				<form:options items="${violationSeasonList}"  itemLabel="optionText" itemValue="optionValue"  />
			</form:select>
				...
				<ul class='table_bd clearfix ${statusObj.index%2==0?"even":"odd"}'style="width:1330px;">
            	   <li class="isKeyCity_Nick_80"><span title="${violation1.isKeyCity_Nick}">${violation1.isKeyCity_Nick}</span></li>
            		...
            		<li class="tiny_operator_100">
						<a href="<%=path%>/dealer/violation/detailSeason.vti?vehicleId=<c:out value="${violation1.vehicleId}" />&driveLineId=
						<c:out value="${violation1.driveLineId}"/>&season=<c:out value="${violation.season}" /> 
						&year=<c:out value="${violation.year}" />&companyName=<c:out value="${violation1.companyName}" />
						...
						&dealerCode=<c:out value="${violation.dealerCode}" />
						&carStyleId_Nick=<c:out value="${violation1.carStyleId_Nick}" /> ">查看明细</a>
					</li>
			</ul>
		</c:forEach>
		</c:if>
