...
function gotoLoadExcel(){
    ...
    pageFormObj.action=	"<%=path%>/dealer/violation/outputDetailIntevel.vti?startIndex=${transMsg.startIndex}&vehicleId=${vehicleId}&driveLineId=${driveLineId}&startTime=${startTime}&endTime=${endTime}&plateNumber=${plateNumber}&typeId_nick=${typeId_nick}&companyName=${companyName}";
        
    pageFormObj.submit();
    pageFormObj.action="<%=path%>/dealer/violation/detailIntevel.vti";
}
</script>
<body>
<form id="myPageForm" action="<%=path%>/dealer/violation/detailIntevel.vti" method="get" >
...
<div class="bd_rt" id="audit">
	...
	<div class="bd_rt_nav clearfix">
		<span>当前位置：违规明细 > 时间段统计 > 违规明细     经销商:${companyName} 车牌:${plateNumber}</span>
	 </div>
	<div class="audit_nav_c">
		 <strong><span><a href="#" onClick="javascript:gotoLoadExcel();">表格导出</a></span></strong>
	     <a href="<%=path%>/dealer/violation/listIntevel.vti?startTime=${startTime}&plateNumber=${plateNumberValue}&endTime=${endTime}&dealerCode=${dealerCode}&vin=${vinValue}";">返回上级</a>
	</div>
