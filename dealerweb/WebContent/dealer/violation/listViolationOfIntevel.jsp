...
function gotoLoadExcel(){
	pageFormObj.action="<%=path%>/dealer/violation/outputIntevel.vti?startIndex=${transMsg.startIndex}";
    pageFormObj.submit();
    pageFormObj.action="<%=path%>/dealer/violation/listIntevel.vti?startIndex=${transMsg.startIndex}";
}

</script>

<body>
<form id="myPageForm" action="<%=path%>/dealer/violation/listIntevel.vti" method="get" >
		...
		<div class="table_list table_all" style="min-width:1200px;overflow-y:hidden;overflow:auto">
			...
			<ul class='table_bd clearfix ${statusObj.index%2==0?"even":"odd"}' style="width:1330px">
            		...
            		<li class="tiny_operator_100">
            			<a href="<%=path%>/dealer/violation/detailIntevel.vti?vehicleId=<c:out value="${violation1.vehicleId}" />
						&driveLineId=<c:out value="${violation1.driveLineId}"/>
						&startTime=<c:out value="${startTime}" />
						&endTime=<c:out value="${endTime}" />&companyName=<c:out value="${violation1.companyName}" /> 
