...
<table width="100%" border="0" cellpadding="2" cellspacing="1"
	...
	<tr>
		<td width="81%" height=23 nowrap class="bgcolor"><strong>管理员日志：</strong></td>
...
<form id="myPageForm" action="/back/operatorLog/search.vti" method="get">
<table width="100%" border="0" cellpadding="2" cellspacing="1"
	class="border">
...
<table width="100%" border="0" align="center" cellpadding="2"
	...
	<tr class="bgcolor">
		<td width="60" height="25" class="bgcolor">管理员ID</td>
	...
	<c:if test="${operatorlogs != null}">
		<c:forEach var="monitorUserLog" items="${operatorlogs}">
			<tr bgcolor="#FFFFFF" onmouseover="this.bgColor='#DFEDEA'"
				onmouseout="this.bgColor='#FFFFFF'">
				 
				 <!--<td align="center"><c:out value="${monitorUserLog}" /></td> -->

				<td><c:out value="${monitorUserLog.operatorId}" /></td>
				...
				<td><fmt:formatDate value="${monitorUserLog.actionTime}" type="both" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				 ...
				 <td title='${monitorUserLog.urlAndParameters}'>
          			<c:out value="${fn:length(monitorUserLog.urlAndParameters) > 60 ? fn:substring(monitorUserLog.urlAndParameters, 0, 60) : monitorUserLog.urlAndParameters}"  escapeXml="false"/>
           			<c:out value="${fn:length(monitorUserLog.urlAndParameters) > 60 ? '...' : ''}"  escapeXml="false"/>
				</td>
		...
		<tr bgcolor="#FFFFFF">
			...
			<div align="right">
			<jsp:include page="/pub/navigate/navigation.jsp"
				flush="true">
				<jsp:param name="pageFormId" value="myPageForm" />
				<jsp:param name="pageFormAct" value="search" />
			</jsp:include></div>
			</td>
		</tr>
	</c:if>
