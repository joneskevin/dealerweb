...
function gotoLoadExcel(){
	pageFormObj.action="<%=path%>/dealer/vehicle/exportVehicle.vti?startIndex=${transMsg.startIndex}";
    pageFormObj.submit();
    pageFormObj.action="<%=path%>/dealer/vehicle/list.vti?startIndex=${transMsg.startIndex}";
}
</script>

<body>
<form id="myPageForm" action="<%=path%>/dealer/vehicle/list.vti" method="get" >
<div class="bd_rt" id="audit">
	...
	<aside class="bd_rt_main clearfix">
		<div class="table_list table_all" >
				<ul class="table_hd clearfix" style="width:1920px;">
            		...
            		<li class="isDemolition_Nick_50">可拆除</li>
			...
			<c:if test="${vehicleList != null}" >
				...
				<ul class='table_bd clearfix ${statusObj.index%2==0?"even":"odd"}' style="width:1920px;">
            		...
            		<li class="isDemolition_Nick_50"><span title="${vehicle.isDemolitionNick}">${vehicle.isDemolitionNick}</span></li>
