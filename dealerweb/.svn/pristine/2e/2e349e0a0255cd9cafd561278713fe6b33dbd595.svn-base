...
function gotoLoadExcel(){
    pageFormObj.action="<%=path%>/dealer/violation/outputDetail.vti?startIndex=${transMsg.startIndex}&vehicleId=${vehicleId}&driveLineId=${driveLineId}&startTime=${startTime}&endTime=${endTime}&plateNumber=${plateNumber}&week=${week}&typeId_nick=${typeId_nick}&companyName=${companyName}";
    pageFormObj.submit();
    pageFormObj.action="<%=path%>/dealer/violation/detail.vti";
}
</script>

<body>
<form id="myPageForm" action="<%=path%>/dealer/violation/detail.vti" method="get" >
...
<div class="bd_rt" id="audit">
	...
	<div class="audit_nav_c">
		...
		<a href="<%=path%>/dealer/violation/listWeek.vti?plateNumber=${plateNumberValue}&year=${year}&week=${week}&dealerCode=${dealerCode}&vin=${vinValue}";">返回上级</a>
	</div>
