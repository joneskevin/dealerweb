...
function gotoLoadExcel(){
    pageFormObj.action="<%=path%>/dealer/violation/outputDetailSeason.vti?startIndex=${transMsg.startIndex}&vehicleId=${vehicleId}&driveLineId=${driveLineId}&startTime=${startTime}&endTime=${endTime}&plateNumber=${plateNumber}&season=${season}&typeId_nick=${typeId_nick}&companyName=${companyName}";
    pageFormObj.submit();
    pageFormObj.action="<%=path%>/dealer/violation/detailSeason.vti";
}
</script>

<body>
<form id="myPageForm" action="<%=path%>/dealer/violation/detailSeason.vti" method="get" >
...
<div class="bd_rt" id="audit">
	...
	<div class="bd_rt_nav clearfix">
		<span>当前位置：违规明细 > 季度统计 > 违规明细    经销商:${companyName} 车牌:${plateNumber}</span>
	 </div>
	<div class="audit_nav_c">
		 <strong><span><a href="#" onClick="javascript:gotoLoadExcel();">表格导出</a></span></strong>
			  <a href="<%=path%>/dealer/violation/listSeason.vti?plateNumber=${plateNumberValue}&year=${year}&season=${season}&dealerCode=${dealerCode}&vin=${vinValue}";">返回上级</a>
	</div>
