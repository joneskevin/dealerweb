package com.ava.base;

import com.sun.jna.Library;
import com.sun.jna.Native;

public class TestJNA {

	public interface IJNALibrary extends Library {

		public abstract int Process(String regFingertpl, String verFingertpl);

		IJNALibrary instance = (IJNALibrary) Native.loadLibrary("Match",
				IJNALibrary.class);
	}

	public static void main(String[] args) throws Exception {
		String regFingertpl = " Gw0N1TEZoczad66 9Kfab6JcuEMPGA165kLR2Z3ejcS ";
		String verFingertpl = "";

		// String regFingertpl =
		// "";
		// String verFingertpl =
		// "";
		System.out.println(IJNALibrary.instance.Process(regFingertpl, verFingertpl));
		System.out.println("success");
	}
}