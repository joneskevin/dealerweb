...
/**Springframework的ObjectUtils,把它拿过来用,自己增加了几个方法
...
public abstract class ObjectUtils {
	 ...
	 * @param o
	 */
	public static void ifNullToPrimitiveValue(Object o){
	...
	public static String objToString(Object o){
		return objToString(o, false, false);
	}
	
	public static String objToString(Object o, boolean parseColl){
		return objToString(o, parseColl, false);
	}
	
	/**将一个Object转换为String
	 * 如果parseColl为false，则不处理集合类型字段
	 * 如果parseBean为false，则不处理映射类型字段
	 * @param o 要toString的对象
	 * @param parseColl 是否处理集合类型字段
	 * @param parseBean 是否处理映射类型字段
	 */
	@SuppressWarnings("unchecked")
	public static String objToString(Object o, boolean parseColl, boolean parseBean){
		if(null == o){
			return "null";
		}
		...
		try{
			if(Collection.class.isAssignableFrom(o.getClass())){
				...
				for(int i=0; i<oa.length; i++){
					...
					sb.append(objToString(oa[i], parseColl));
				}
				sb.append(" ] ");
			}else if(o.getClass().isArray()){
				Object[] oa = (Object[]) o;
				sb.append("Array:"+oa.length+" [ ");
				for(int i=0; i<oa.length; i++){
					if(i>0){
						sb.append(", ");
					}
				...
				while(ite.hasNext()){
					Object key = ite.next();
					sb.append(key+" : "+objToString(map.get(key), parseColl));
				}
				sb.append(" } ");
			}else if(ClassUtils.isMappingType(o.getClass())){
				sb.append("\r\n"+o.getClass().getName()+": { ");
				for(Class clazz = o.getClass(); clazz != null && (clazz != Object.class); clazz = clazz.getSuperclass()){
					Field[] fields = clazz.getDeclaredFields();
					appendFieldValue(sb, o, fields, parseColl, parseBean);
				}
				sb.append(" } \r\n");
			}else{
	 ...
	 * 调用传入的对象的方法得到其返回的值
	 */
	private static void appendFieldValue(StringBuilder sb, Object o, Field[] fields, boolean parseColl, boolean parseBean) throws Exception{
		...
		for(int j=0;j<fields.length;j++){
			...
			try{
				if(parseColl && parseBean){
					sb.append("["+f.getName()+":"+f.get(o)+"]");
				}else{
					if(parseBean && !parseColl){
						if(!Collection.class.isAssignableFrom(f.getType())){
							sb.append("["+f.getName()+":"+f.get(o)+"]");
						}
					}else if(!parseBean && parseColl){
						if(! ClassUtils.isMappingType(f.getType())){
							sb.append("["+f.getName()+":"+f.get(o)+"]");
						}
					}else{
						if(! ClassUtils.isMappingType(f.getType())
