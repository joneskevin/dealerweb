...
<script>
	...
	function gotoLoadExcel(){
		pageFormObj.action="<%=path%>/dealer/testDrive/outputMonth.vti?startIndex=${transMsg.startIndex}";
		pageFormObj.submit();
		pageFormObj.action="<%=path%>/dealer/testDrive/listMonth.vti";
	}    
</script>

<body>
<form id="myPageForm" action="<%=path%>/dealer/testDrive/listMonth.vti" method="get" >
	...
	<div class="audit_nav_c">
		...
		<select id="year" name="year" class="select_condition">
			<option value=<%=nowYear%>
				<%=chooseYear.equals(nowYear) ? "selected='selected' " : ""%>>
				<%=nowYear%>
			</option>
			<option value=<%=lastYear%>
				<%=chooseYear.equals(lastYear) ? "selected='selected' " : ""%>>
				<%=lastYear%>
			</option>
		</select> 
		<form:select path='testDrive.month' cssClass="select_condition">
			<form:options items="${testDriveMonthList}" itemLabel="optionText" itemValue="optionValue" />
		</form:select>
					...
					<ul class='table_bd clearfix ${statusObj.index%2==0?"even":"odd"}'style="width:1360px;">
	            		...
	            		<li class="short_operator_150">
							<a href="<%=path%>/dealer/testDrive/listTestDriveDetail.vti?vinValue=${testDrive.vehicle.vin}						
							 ...
							 &vehicleId=${vo.vehicleId}&lineId=${vo.lineId}&year=${testDrive.year}&month=${testDrive.month}&higherPage=${transMsg.startIndex}">查看明细</a>
	            		</li>
					</ul>
				</c:forEach>
			</c:if>
